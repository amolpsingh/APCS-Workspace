...
/**
 * A class for an erase object, that utilizes recursive methods
 * and employs a world.
 *
 * @author asingh989
 * @version 11/13/17
...
public class EraseObject extends World<Tile>
{
    ...
    private void getData( String fName )
    {
        ...
        {
            System.out.println( "Input file will not open" );
            return;
        }
    ...
    private void erase( Grid<Tile> gr, int row, int col )
    {
        ...
        {
            if ( gr.get( location ) != null && gr.get( location ).getColor().equals( IMGOBJ_COLOR ) )
            {
                gr.put( location , new Tile( ERASE_COLOR ) );
